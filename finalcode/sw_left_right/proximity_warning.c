...
void load_example_data(void)
{
    ...
    for (uint32_t i = 0; i < 500; i++) {
        ...
        radar_data[i].rpm = 400 + (simple_rand() % 1001);   // high RPM values
 ...
 *          MODIFIED FOR HIGH SPEED & FAST TURNING
 ***********************************************************/
static void update_fallback_logic(uint32_t dist_cm, uint32_t rpm, int32_t x_mean_q16, int32_t obj_vel_cms)
{
    // 1. Compute base duty – now always high unless very close or braking
    uint32_t obj_approaching = (obj_vel_cms < 0) && ((uint32_t)(-obj_vel_cms) > 50);
    
    // Force high speed regardless of reported RPM (treat as always moving)
    // Only reduce duty when object is dangerously close.
    if (dist_cm > RANGE_7M) {
        base_duty = DUTY_100;                // max speed
    } else if (dist_cm > RANGE_5M) {
        base_duty = obj_approaching ? DUTY_80 : DUTY_100;
    } else if (dist_cm > RANGE_3M) {
        base_duty = obj_approaching ? DUTY_60 : DUTY_80;
    } else {
        base_duty = DUTY_20;                 // slow down very close
    }

    // 2. Lane-change steering – increased differential for faster turning
    ...
    // Larger reduction amounts -> sharper turn
    uint32_t reduce_amount = hard_steer ? (PWM_PERIOD * 70 / 100)   // was 40%
                                         : (PWM_PERIOD * 50 / 100);  // was 20%

    uint32_t alert_now = (dist_cm < RANGE_7M) ? 1 : 0;

    switch (lc_state) {
        case 0: // LC_STRAIGHT
            steer_state = 0;
            if (alert_now && (object_right || object_left)) {
    ...
    // Compute left and right duties – apply large reduction for fast turn
    duty_left = base_duty;
    duty_right = base_duty;
    if (lc_state == 1) { // TURN phase
        if (steer_dir == 0) {   // turning left -> reduce right motor
            if (duty_right > lc_reduce) duty_right -= lc_reduce;
            else duty_right = 0;
        } else {                // turning right -> reduce left motor
            if (duty_left > lc_reduce) duty_left -= lc_reduce;
            else duty_left = 0;
        }
    } else if (lc_state == 3) { // CORRECT phase (opposite wheel reduction)
        if (steer_dir == 0) {   // correct from left turn -> reduce left motor
            if (duty_left > lc_reduce) duty_left -= lc_reduce;
            else duty_left = 0;
        } else {                // correct from right turn -> reduce right motor
            if (duty_right > lc_reduce) duty_right -= lc_reduce;
            else duty_right = 0;
        }
    }

    // 3. Buzzer pattern (unchanged)
...
int main(void)
{
    ...
    while (1)
    {
        ...
        if (ip_present) {
            // Hardware mode – unchanged except thresholds already set
            ...
            // Software emulation mode – using modified high‑speed logic
